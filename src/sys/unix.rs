...
impl Domain {
    ...
    #[cfg(all(
        feature = "all",
        any(target_os = "android", target_os = "fuchsia", any(target_os = "linux", target_os = "runixos"))
    ))]
    pub const PACKET: Domain = Domain(libc::AF_PACKET);

    /// Domain for low-level VSOCK interface, corresponding to `AF_VSOCK`.
    #[cfg(all(feature = "all", any(target_os = "android", any(target_os = "linux", target_os = "runixos"))))]
        ...
        self.as_sockaddr_un()
            .map(|storage| {
                self.len() == offset_of_path(storage) as _
                    ...
                    || (cfg!(not(any(any(target_os = "linux", target_os = "runixos"), target_os = "android", target_os = "cygwin")))
